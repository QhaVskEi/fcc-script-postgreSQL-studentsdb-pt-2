...
echo "$($PSQL "SELECT course FROM students RIGHT JOIN majors USING(major_id) INNER JOIN majors_courses USING(major_id) INNER JOIN courses USING(course_id) WHERE student_id IS NOT NULL GROUP BY course HAVING COUNT(course)=1 ORDER BY course ASC")"
